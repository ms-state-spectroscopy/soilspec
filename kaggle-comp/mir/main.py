...
def loadKaggleData(train_split=0.75, from_pkl=True):

    store = pd.HDFStore("kaggle-comp/mir/store.h5")

    print("Loading training data... ", end="")
    if from_pkl:
        train_df: pd.DataFrame = store["train"]
    else:
        train_df = pd.read_csv("kaggle-comp/mir/train.csv")
    ...
    X_train_np -= X_train_np.min(axis=1)[:, np.newaxis]
    X_train_np /= X_train_np.max(axis=1)[:, np.newaxis]

    X_train = pd.DataFrame(data=X_train_np, columns=X_train.columns)

    Y_train = train_df["clay_perc"]

    X_test = pd.read_csv("kaggle-comp/mir/test.csv").set_index("unique_id").iloc[:, 2:]

    X_test_np = X_test.to_numpy()
    X_test_np -= X_test_np.min(axis=1)[:, np.newaxis]
    X_test_np /= X_test_np.max(axis=1)[:, np.newaxis]

    X_test = pd.DataFrame(data=X_test_np, columns=X_test.columns, index=X_test.index)
...
if __name__ == "__main__":
    ...
    Y_pred_mlp = pd.DataFrame(
        mlp_analyzer.predict(X_test), columns=["clay_perc"], index=X_test.index
    )
    print(Y_pred_mlp)
